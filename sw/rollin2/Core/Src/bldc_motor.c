...
uint16_t bldc_motor_move(bldc_motor_t *m, float dt) {
    ...
    switch(m->ctrl.type) {
        case BLDC_MOTOR_CTRL_TYPE_VELOCITY_OPENLOOP: 
            bldc_motor_velocity_openloop(m);
            break;
        case BLDC_MOTOR_CTRL_TYPE_ANGLE_OPENLOOP: 
            bldc_motor_angle_openloop(m);
            break;
    }
